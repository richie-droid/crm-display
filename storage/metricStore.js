...
function readStore() {
  ...
  try {
    ...
    throw new Error(
      `Unable to read metric store: ${error.message}`
    );
  }
}
...
function saveSnapshot({
  ...
  if (existingIndex >= 0) {
    ...
    const existingIsHistorical =
      isHistoricalSource(existing.source);
...
function getHistory(metricKey, days = 365) {
  ...
  return readStore()
    ...
    .filter(
      (item) =>
        item.metricKey === metricKey &&
        new Date(item.capturedAt) >= cutoff
    )
    .sort(
      (a, b) =>
        new Date(a.capturedAt) -
