...
function renderHomeStretchAdminPage({ roster, weeks, callsData, adjustmentsData }) {
  ...
  const headerCells = weeks
    .map(
      (week) =>
        `<th class="week" title="Week of ${escapeHtml(week.weekStart)}">${dateLabel(week.weekStart)}</th>`
    )
    .join("");
    ...
    .map((agent) => {
      ...
      return `<tr>
        ...
        <td class="agent-cell">
          ...
          <span>${escapeHtml(agent.salesforceName)}${agent.handicap !== 1 ? ` &middot; ${Math.round(agent.handicap * 100)}% handicap` : ""}</span>
        </td>
        <td class="adj-cell">
          <input
...
<head>
  ...
  <title>The Home Stretch - Call Admin</title>
  <style>
    :root {
      color-scheme: dark;
      --bg: #0f1720;
    ...
    * { box-sizing: border-box; }
    ...
    .call-input:hover, .call-input:focus { border-color: var(--accent); outline: none; }
    .adjustment-input {
      width: 100%;
      border: 1px solid transparent;
      border-radius: 5px;
    ...
    .adjustment-input:hover, .adjustment-input:focus { border-color: var(--accent); outline: none; }
    .notes-input {
      width: 100%;
      border: 1px solid transparent;
      border-radius: 5px;
    ...
    .notes-input:hover, .notes-input:focus { border-color: var(--accent); outline: none; }
...
<body>
  <main class="page">
    <div class="header">
      <div>
        <h1>The Home Stretch - Call Admin</h1>
    ...
    <div class="legend">${weeks.length} weekly columns, Sep 14 through Nov 25.</div>
  </main>
  <script>
    ...
    saveButton.addEventListener("click", async () => {
      ...
      try {
        const [callsResponse, adjustmentsResponse] = await Promise.all([
          fetch("/api/home-stretch/calls", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ entries }),
          }),
          fetch("/api/home-stretch/adjustments", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
...
module.exports = { renderHomeStretchAdminPage };
